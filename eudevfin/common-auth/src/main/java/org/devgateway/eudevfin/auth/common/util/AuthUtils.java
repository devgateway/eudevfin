...
public final class AuthUtils {
    ...
    public static PersistedUser getCurrentUser() {
        ...
        if (principal instanceof PersistedUser)
            return (PersistedUser) principal;
        return null;
    }
    ...
    public static boolean userHasRole(PersistedUser user, String role) {
        for (PersistedAuthority authority : user.getPersistedAuthorities())
    ...
    /**
     * Retrieves the team leader of the organization that the current
     * user is belonging to
     *
     * @return {@link PersistedUser} team leader or null if no team leader is set
     */
    public static PersistedUser currentUsersTeamLead() {
        PersistedUser user = getCurrentUser();
        Set<PersistedUser> users = user.getGroup().getUsers();
        for (PersistedUser u : users) {
            if (userHasRole(u, AuthConstants.Roles.ROLE_TEAMLEAD))
                return u;
        }
        return null;
    }
