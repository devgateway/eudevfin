...
//		assertNotNull(rootSectorsInitialized.getTags().iterator().next());
	...
	private void createFakeSectors() {
		...
		sectorsRoot.setChildren(new HashSet<Category>());
		...
		sectorsRoot.getChildren().add(sector1);
		...
		sectorsRoot.getChildren().add(sector2);
	...
	private void createLabels() {
		...
		labelRoot.setChildren(new HashSet<Category>());
		
		Category sectors = new Category();
		sectors.setName("Sectors Label - test");
		sectors.setCode(SECTORS_LABEL_TEST);
		sectors.setChildren(new HashSet<Category>());
		labelRoot.getChildren().add(sectors);
		...
		labelRoot.getChildren().add(fakeLabel);
