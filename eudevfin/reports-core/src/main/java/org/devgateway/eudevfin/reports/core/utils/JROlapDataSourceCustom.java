...
/**
 * Based on original net.sf.jasperreports.olap.JROlapDataSource 
 * with customization for avoiding exceptions when tuple is not 
 * found. Changes in:
 * getTuplePosition()
 * DataFieldMatcher.value()
 */
public class JROlapDataSourceCustom implements JRDataSource, MappingMetadata
{
	private static final Log log = LogFactory.getLog(JROlapDataSourceCustom.class);
	...
	public JROlapDataSourceCustom(JRDataset dataset, JROlapResult result)
	{
	...
	public int getTuplePosition(int axisIndex, Tuple tuple)
	{
		...
		if (pos == -1)
		{
			...
			log.warn("No such tuple " + sb + " on axis " + axisIndex + ".");
		}
		return pos;
	}
}
