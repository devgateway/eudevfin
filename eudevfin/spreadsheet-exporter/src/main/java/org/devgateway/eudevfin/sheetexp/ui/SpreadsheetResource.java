...
public class SpreadsheetResource extends AbstractResource {
	...
	public SpreadsheetResource(final SpreadsheetTransformerService transformerService, final CustomFinancialTransactionService txService) {
		super();
...
//						new EntityWrapperListHelper<FinancialTransaction>(txList, MetadataConstants.FSS_REPORT_TYPE, now).addToWrappedList(finalList);
						...
						final List<CustomFinancialTransaction> txList		= SpreadsheetResource.this.txService.findByReportingYearAndDraftFalse(2013);
						new EntityWrapperListHelper<CustomFinancialTransaction>(txList, MetadataConstants.FSS_REPORT_TYPE, now, "en").addToWrappedList(finalList);
						
						SpreadsheetResource.this.transformerService.createSpreadsheetOnStream(finalList, finalList.size(), response.getOutputStream() );
